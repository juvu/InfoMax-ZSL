...
class MINE(nn.Module):
    def __init__(self, opt):
        ...
        self.x_size = opt.resSize
        self.z_size = opt.resSize if opt.visual_mi else opt.attSize
        self.fc1_x = nn.Linear(self.x_size, opt.m_nh, bias=False)
        self.fc1_z = nn.Linear(self.z_size, opt.m_nh, bias=False)

        self.fc1_bias = nn.Parameter(torch.zeros(opt.m_nh))
        self.lrelu1 = nn.LeakyReLU(opt.lrelu, True)
        self.fc2 = nn.Linear(opt.m_nh, opt.m_nh)
        self.lrelu2 = nn.LeakyReLU(opt.lrelu, True)
        self.fc3 = nn.Linear(opt.m_nh, opt.m_no)
        self.lrelu3 = nn.LeakyReLU(opt.lrelu, True)
    ...
    def forward(self, x, z):
        x = self.fc1_x(x)
        z = self.fc1_z(z)
        y = self.lrelu1(x + z + self.fc1_bias)
        y = self.lrelu2(self.fc2(y))
        y = self.lrelu3(self.fc3(y))
        return y
